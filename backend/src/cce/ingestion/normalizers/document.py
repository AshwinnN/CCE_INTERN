...
cce.connectors.base.canonical_types.canonicalize_type() as data, not branched on.
"""
...
def normalize_to_canonical(payload: Optional[Dict[str, Any]], kind: str,
    ...
    """
    payload:
      - unstructured: the parsed_doc dict from cce.ingestion.parsers (already a
      ...
      - structured: the schema-card dict returned by
        cce.connectors.base.connector.StructuredConnector.get_schema_card()
        (via cce.connectors.fetch.fetch_structured()), shaped
