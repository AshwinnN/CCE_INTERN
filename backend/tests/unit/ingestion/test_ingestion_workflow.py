...
from cce.ingestion.orchestrator import (  # noqa: E402
...
from cce.persistence.ports import MetadataRepository, SchemaSnapshot  # noqa: E402
...
class FetchStructuredNodeTests(unittest.TestCase):
    ...
    def test_no_injected_fetcher_uses_the_real_default_for_snowflake(self):
        ...
        with mock.patch("cce.connectors.fetch.snowflake_schema_fetcher",
...
class PersistStructuredMetadataNodeTests(unittest.TestCase):
    ...
    def test_no_repository_configured_is_a_warning_not_an_error(self):
        os.environ.pop("CCE_CONTROL_DATABASE_URL", None)
        ...
        result = persist_structured_metadata_node(state)
        self.assertEqual(result["errors"], [])
        self.assertIn("CCE_CONTROL_DATABASE_URL not configured", result["warnings"][0])
        self.assertIsNone(result["metadata_snapshot_id"])
