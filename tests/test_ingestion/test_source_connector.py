...
from common.tools.source_connector import (  # noqa: E402
...
class SnowflakeSchemaFetcherTests(unittest.TestCase):
    ...
    def test_connects_fetches_the_schema_card_and_always_closes(self):
        ...
        with mock.patch("connectors.snowflake.config.build_config_from_env", return_value=fake_config), \
             mock.patch("connectors.factory.ConnectorFactory.create", return_value=FakeConnector(fake_config)):
    ...
    def test_closes_the_connector_even_when_get_schema_card_raises(self):
        ...
        fake_config = mock.Mock(schema="PUBLIC")
        with mock.patch("connectors.snowflake.config.build_config_from_env", return_value=fake_config), \
             mock.patch("connectors.factory.ConnectorFactory.create", return_value=FailingConnector(fake_config)):
            fetcher = snowflake_schema_fetcher()
            with self.assertRaises(RuntimeError):
